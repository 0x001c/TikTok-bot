    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                ...
                if DebugMode:
                    print(Colorate.Horizontal(Colors.red_to_blue, f"Sent Share: {SentTotalSentShares} ({impr_id})"))
                else:
                    print(Colorate.Horizontal(Colors.red_to_blue, f"Sent Share: {SentTotalSentShares} ({impr_id})"))
...
if __name__ == "__main__":
    Clear()
    itemID = Write.Input("Pasted by 0x001c | Video here > ", Colors.red_to_green, interval=0.0001)
    amount = Write.Input("Amount (0=inf) > ", Colors.white_to_green, interval=0.0001)
    NThread = Write.Input("Thread Amount > ", Colors.white_to_green, interval=0.0001)

    if Title("Proy Scrapper X-Proxy by NightFallGT"):
        Debug = Write.Input("Debug Fails [y/n] ? > ", Colors.green_to_red, interval=0.0001)
